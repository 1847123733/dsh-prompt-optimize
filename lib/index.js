...
export const inject = ['credentials', 'llm', 'settings', 'systemPrompt', 'webServer']
...
const PERSONALIZATION_API_PATH = '/api/prompt-optimize/personalization'
const PERSONALIZATION_SETTINGS_NAMESPACE = 'prompt-optimize-personalization'
const PERSONALIZATION_PROMPT_SECTION = 'prompt-optimize:custom-instructions'
const PERSONALIZATION_MAX_CHARS = 32_000
...
function personalizationPrompt(value) {
  const instructions = normalizeCustomInstructions(value?.customInstructions)
  if (!instructions) return ''
  return [
    '# Custom Instructions',
    '',
    'The user configured the following host-wide instructions. Apply them to every task unless they conflict with higher-priority instructions:',
    '',
    instructions,
  ].join('\n')
}
...
export function apply(ctx, config) {
  const llm = ctx.llm
  const personalizationSettings = ctx.settings?.register?.(
    PERSONALIZATION_SETTINGS_NAMESPACE,
    PersonalizationSettingsSchema,
    { base: { customInstructions: '' }, applies: 'live' },
  )

  if (personalizationSettings) {
    ctx.effect(
      () =>
        personalizationSettings.watch(() => {
          ctx.emit?.('system-prompt/change')
        }),
      'dsh-prompt-optimize: personalization settings watch',
    )
  }
    ...
    ctx.effect(
      () =>
        ctx.systemPrompt.section({
          name: PERSONALIZATION_PROMPT_SECTION,
          order: 10,
          text: () => personalizationPrompt(personalizationSettings?.get()),
        }),
      'dsh-prompt-optimize: custom instructions system prompt',
    )
  }
  ...
  ctx.effect(() =>
    ctx.webServer.register({
      ...
      handler: async (req, res) => {
        ...
        if (req.method === 'GET') {
          json(res, 200, {
            customInstructions: personalizationSettings.get().customInstructions,
          })
          return
        }
        ...
        if (typeof body?.customInstructions !== 'string') {
          json(res, 400, { error: 'customInstructions must be a string' })
          return
        }
        const customInstructions = body.customInstructions.replace(/\r\n?/g, '\n')
        if (customInstructions.length > PERSONALIZATION_MAX_CHARS) {
          json(res, 400, {
            error: `自定义指令不能超过 ${PERSONALIZATION_MAX_CHARS} 个字符。`,
          })
          return
        }

        try {
          await personalizationSettings.replace({ customInstructions })
          json(res, 200, { customInstructions })
        } catch (error) {
          json(res, 500, {
