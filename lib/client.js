  ...
  factory: (require) => {
    ...
    if (
      ...
      styleTag.textContent = [
        ...
        '.po-usage{position:fixed;right:18px;bottom:18px;z-index:45;pointer-events:auto;font-family:ui-sans-serif,system-ui,-apple-system,sans-serif;}',
        ...
        '.po-usage-card{position:absolute;right:0;bottom:calc(100% + 10px);width:286px;box-sizing:border-box;padding:14px;border:1px solid var(--border-subtle,var(--dsw-alias-border-l2,rgba(0,0,0,.1)));border-radius:16px;background:var(--bg-surface,var(--dsw-alias-bg-overlay,#fff));color:var(--fg-primary,var(--dsw-alias-label-primary,#111));box-shadow:0 18px 50px rgba(0,0,0,.2);}',
    ...
    function preferredUsageCost(usage, balance) {
      var costs = usage && usage.costs && typeof usage.costs === 'object' ? usage.costs : {}
      var currency = balance && balance.currency ? balance.currency : 'CNY'
      if (Number.isFinite(Number(costs[currency]))) {
        return { currency: currency, value: Number(costs[currency]) }
      }
      var names = Object.keys(costs)
      return names.length
        ? { currency: names[0], value: Number(costs[names[0]]) }
    ...
    function DeepSeekUsageFloat() {
      ...
      var cost = preferredUsageCost(data && data.usage, balance)
      var rangeDays = data && data.range ? data.range.days : 30
      var compact =
        data && data.usage && cost.value !== null
          ? rangeDays + '天 ' + formatUsageMoney(cost.value, cost.currency)
          : 'DeepSeek 用量'
          ...
          ? h(
              ...
              h(
                ...
                h(
                  'div',
                  null,
                  h('div', { className: 'po-usage-title' }, 'DeepSeek API 用量'),
                  h('div', { className: 'po-usage-key' }, data && data.key ? data.key : 'DEEPSEEK_API_KEY'),
                ),
                h(
                  'button',
                  {
              ...
              phase === 'error'
                ...
                : h(
                    ...
                    h(
                      ...
                      h('div', { className: 'po-usage-label' }, '近 ' + rangeDays + ' 天已消费'),
                      h(
                        'div',
                        { className: 'po-usage-money' },
                        data && data.usage && cost.value !== null
                          ? formatUsageMoney(cost.value, cost.currency)
                          : phase === 'loading'
                            ? '加载中…'
                            : '暂不可用',
                      ),
                    ...
                    h(
                      ...
                      h(
                        ...
                        h('span', { className: 'po-usage-label' }, '当前余额'),
                        h(
                          'strong',
                          null,
                          balance
                      ...
                      h(
                        'div',
                        { className: 'po-usage-stat' },
                        h('span', { className: 'po-usage-label' }, '请求 / Tokens'),
                        h(
                          'strong',
                          null,
                          data && data.usage
                            ? Number(data.usage.requests || 0).toLocaleString() +
                                ' / ' +
                                Number(data.usage.tokens || 0).toLocaleString()
                            : '—',
                        ),
                      ),
                    ),
                    data && data.usageUnavailable
                      ? h(
                          'div',
                          { className: 'po-usage-note' },
                          data.usageMessage || 'DeepSeek 用量接口暂不可用；余额仍可正常查询。',
                        )
                      : h(
                          'div',
                          { className: 'po-usage-note' },
                          '每 60 秒自动刷新 · 完整 API Key 仅保留在 DSH 宿主端',
                        ),
                  ),
            )
          : null,
