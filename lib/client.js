...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var PERSONALIZATION_API_PATH = '/api/prompt-optimize/personalization'
    var PERSONALIZATION_MAX_CHARS = 32000
    ...
    if (
      ...
      styleTag.textContent = [
        ...
        '.po-personalization{box-sizing:border-box;width:100%;max-width:820px;padding:4px 0 24px;color:var(--fg-primary,var(--dsw-alias-label-primary,#111));font-family:ui-sans-serif,system-ui,-apple-system,sans-serif;}',
        '.po-personalization-head{display:flex;align-items:flex-start;justify-content:space-between;gap:20px;margin-bottom:18px;}',
        '.po-personalization-title{margin:0;font-size:16px;line-height:24px;font-weight:650;}',
        '.po-personalization-desc{margin:4px 0 0;color:var(--fg-secondary,var(--dsw-alias-label-secondary,#666));font-size:12px;line-height:20px;}',
        '.po-personalization-save{min-width:58px;height:30px;padding:0 14px;border:0;border-radius:8px;background:var(--dsw-alias-state-business-primary,#4d6bfe);color:#fff;font-size:12px;font-weight:600;cursor:pointer;}',
        '.po-personalization-save:hover:not(:disabled){filter:brightness(1.06);}',
        '.po-personalization-save:disabled{background:var(--bg-muted,var(--dsw-alias-interactive-bg-disabled,#f2f3f5));color:var(--fg-secondary,var(--dsw-alias-label-disable,#aaa));cursor:not-allowed;}',
        '.po-personalization-textarea{box-sizing:border-box;display:block;width:100%;min-height:220px;resize:vertical;padding:12px 14px;border:1px solid var(--border-subtle,var(--dsw-alias-border-l2,rgba(0,0,0,.12)));border-radius:12px;outline:none;background:var(--bg-surface,var(--dsw-alias-bg-layer-1,#fff));color:var(--fg-primary,var(--dsw-alias-label-primary,#111));font:13px/1.65 ui-sans-serif,system-ui,-apple-system,sans-serif;box-shadow:0 1px 2px rgba(0,0,0,.02) inset;}',
        '.po-personalization-textarea:focus{border-color:var(--dsw-alias-state-business-primary,#4d6bfe);box-shadow:0 0 0 3px rgba(77,107,254,.10);}',
        '.po-personalization-textarea:disabled{opacity:.65;cursor:wait;}',
        '.po-personalization-foot{display:flex;align-items:center;justify-content:space-between;gap:16px;min-height:22px;margin-top:7px;color:var(--fg-secondary,var(--dsw-alias-label-tertiary,#888));font-size:11px;}',
        '.po-personalization-status[data-error="1"]{color:#b42318;}',
      ].join('\n')
    ...
    function PersonalizationSettingsSection() {
      ...
      var statusPair = useState('')
      var status = statusPair[0]
      var setStatus = statusPair[1]
      var errorPair = useState(false)
      var hasError = errorPair[0]
      var setHasError = errorPair[1]

      useEffect(function () {
        var controller = new AbortController()
        fetch(PERSONALIZATION_API_PATH, {
          credentials: 'same-origin',
          ...
          .then(function (data) {
            var instructions = typeof data.customInstructions === 'string'
      ...
      var tooLong = value.length > PERSONALIZATION_MAX_CHARS
      var canSave = !loading && !saving && changed && !tooLong
      ...
      return h(
        'section',
        { className: 'po-personalization', 'aria-labelledby': 'po-personalization-title' },
        h(
          'div',
          { className: 'po-personalization-head' },
          h(
            'div',
            null,
            h('h2', { id: 'po-personalization-title', className: 'po-personalization-title' }, '自定义指令'),
            h(
              'p',
              { className: 'po-personalization-desc' },
              '为此主机上的所有任务向 AI 提供额外说明和上下文。保存后会自动注入系统上下文。',
            ),
          ),
          h(
            'button',
            {
              type: 'button',
              className: 'po-personalization-save',
              disabled: !canSave,
              onClick: save,
            },
            saving ? '保存中…' : '保存',
        ...
        h('textarea', {
          className: 'po-personalization-textarea',
          value: value,
          disabled: loading,
          maxLength: PERSONALIZATION_MAX_CHARS + 1,
          placeholder: '例如：\n- 优先使用中文回答。\n- 修改代码前先阅读相关文件。\n- 完成后说明改动与验证结果。',
          'aria-label': '自定义指令',
        ...
        h(
          'div',
          { className: 'po-personalization-foot' },
          h(
            'span',
            { className: 'po-personalization-status', 'data-error': hasError || tooLong ? '1' : '0' },
            loading
              ? '正在加载…'
              : tooLong
                ? '已超过最大字符数'
          ...
          h('span', null, value.length + ' / ' + PERSONALIZATION_MAX_CHARS),
        ),
      )
    }
    ...
    function apply(ctx) {
      ...
      ctx.effect(function () {
        var disposeOptimize = ctx.slots.inject('conversation.input.right', function () {
        ...
        var disposePersonalization = ctx.slots.inject('settings.section', function () {
          return ctx.slots.register(
            {
        ...
        var disposeFileBtn = ctx.slots.inject('conversation.input.left', function () {
          return ctx.slots.register(
            {
        ...
        return function () {
          ...
          if (typeof disposePersonalization === 'function') disposePersonalization()
